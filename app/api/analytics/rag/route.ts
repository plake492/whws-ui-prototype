...
export async function GET(req: NextRequest) {
  try {
    ...
    // Build collection filter - only if collection column exists
    ...
    // 3. Collection breakdown - return empty if no collection column
    const collections = [];
    ...
    // 6. Quality metrics - return empty if no quality columns
    const quality = [];
